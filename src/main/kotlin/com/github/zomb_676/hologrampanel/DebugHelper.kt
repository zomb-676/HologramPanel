        ...
        fun renderLevelLast(event: RenderLevelStageEvent) {
            ...
            if (Config.Client.renderDebugHologramLifeCycleBox.get() && (queryUpdateData.isNotEmpty() || popUpData.isNotEmpty() || removeData.isNotEmpty())) {
                ...
                if (queryUpdateData.isNotEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        val next = iterator.next()
                        val color = FastColor.ARGB32.lerp((next.intValue + partialTick) / UPDATE_TINE, UPDATE_END_COLOR, UPDATE_BEGIN_COLOR)
                ...
                if (popUpData.isNotEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        val next = iterator.next()
                        val color =
                            FastColor.ARGB32.lerp((next.intValue + partialTick) / POPUP_TIME, POP_UP_END_COLOR, POP_UP_BEGIN_COLOR)
                ...
                if (removeData.isNotEmpty()) {
                    ...
                    while (iterator.hasNext()) {
                        val next = iterator.next()
                        val color =
                            FastColor.ARGB32.lerp((next.intValue + partialTick) / REMOVE_TIME, REMOVE_END_COLOR, REMOVE_BEGIN_COLOR)
                        fill(next.key.sourcePosition(partialTick), color, pose, builder)
                    }
                }
