...
sealed interface HologramContextPrototype {
    ...
    class BlockHologramPrototype(val pos: BlockPos) : HologramContextPrototype {
        ...
        companion object {
            val CODEC: MapCodec<BlockHologramPrototype> = RecordCodecBuilder.mapCodec { ins ->
                ins.group(
    ...
    class EntityHologramPrototype(val entityUUID: UUID) : HologramContextPrototype {
        ...
        companion object {
            val CODEC: MapCodec<EntityHologramPrototype> = RecordCodecBuilder.mapCodec { ins ->
                ins.group(
    ...
    enum class PrototypeEnum : StringRepresentable {
        BLOCK {
            override fun codec(): MapCodec<BlockHologramPrototype> = BlockHologramPrototype.CODEC

        },
        ENTITY {
            override fun codec(): MapCodec<EntityHologramPrototype> = EntityHologramPrototype.CODEC
        };

        override fun getSerializedName(): String = this.name
        abstract fun codec(): MapCodec<out HologramContextPrototype>
